...
def targeted_category_discount(
    category:str = Query(..., description="Electronics or Stationery"),
    discount_percent:int = Query(..., ge=1, le=99, description="Discount number")
    ):
    res = []
    for p in products:
    ...
    return {
        ...
        'total_updated_products': len(res),
        'updated_prices': res
    }
...
# Q5 - GET products/audit - Inventory Summary
@app.get("/products/audit")
def get_product_audit():
    in_stock = [p for p in products if p["in_stock"]]
    out_stock = [p for p in products if not p["in_stock"]]
    out_stock_names = [i["name"] for i in out_stock]
    total_instock_value = sum(p["price"] * 10 for p in in_stock)
    most_expensive = max(products, key=lambda p:p["price"])
    return {
        "total_products": len(products), 
        "in_stock_count": len(in_stock), 
        "out_of_stock_names": out_stock_names, 
        "total_stock_value": total_instock_value, 
